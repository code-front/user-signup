...
def signup():
    ...
    validated_password = request.form['validated_password']
    ...
    if validated_password == '':
        validated_password_error = "Please confirm your password"
    if validated_password != password:
        validated_password_error = "Passwords don't match"
    if email != '' and (email.count("@") != 1 or email.count(".") != 1 or is_invalid(email)):
        email_error = "That's not a valid email"


    if not (username_error or password_error or validated_password_error or email_error):
        return redirect('/welcome?username=' + username)
    else:
        return render_template('index.html', username = username,
            email = email, username_error = username_error, password_error = password_error,
            validated_password_error = validated_password_error, email_error = email_error)
